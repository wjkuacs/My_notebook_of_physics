...
\begin{equation*}
    i\hbar \frac{\partial}{\partial t}\phi = \left(-\hbar^2c^2\nabla^2 + m^2 c^4\right)^{\frac{1}{2}} \phi
\end{equation*}
...
\begin{itemize}
    ...
    \item Step 4：通过产生湮灭算符激发场，这将会使用到占有数
    \item Step 5：计算哈密顿量并讨论真空态
\end{itemize}
...
\begin{itemize}
    ...
    \begin{align*}
        ...
        \nabla \hat{\phi}(x) &= \int \frac{d^3p}{\left(2\pi\right)^{\frac{3}{2}}}\frac{1}{\left(2E_{\vec{p}}\right)^{\frac{1}{2}}}\left(i\vec{p}\right)\left(\hat{a}_{\vec{p}}e^{-ip\cdot x} + \hat{a}_{\vec{p}}^\dagger e^{ip\cdot x}\right)
    \end{align*}
...
在定义了这个作用之后，我们重新给出经过normal ordering的哈密顿量
\begin{equation*}
    \hat{H} = \frac{1}{2}\int d^3p E_{\vec{p}}\left(\hat{a}_{\vec{p}}\hat{a}_{\vec{p}}^\dagger + \hat{a}_{\vec{p}}^\dagger \hat{a}_{\vec{p}}\right)
\end{equation*}
...
\section{\protect\hyperlink{:}{传播子 (Propagator)}}
\addtocontents{toc}{\protect\hypertarget{:}{}}

在量子场论中，传播子是一个非常重要的概念，它描述了粒子从一个时空点传播到另一个时空点的量子振幅。对于Klein-Gordon场，我们定义费曼传播子 (Feynman propagator) 为
\begin{equation*}
    D_F(x-y) = \langle 0 | T\left\{\hat{\phi}(x)\hat{\phi}(y)\right\} | 0 \rangle
\end{equation*}

其中 $T$ 是时间序算符 (time-ordering operator)，其定义为
\begin{equation*}
    T\left\{\hat{\phi}(x)\hat{\phi}(y)\right\} = 
    \begin{cases}
        \hat{\phi}(x)\hat{\phi}(y) & \text{if } x^0 > y^0 \\
        \hat{\phi}(y)\hat{\phi}(x) & \text{if } y^0 > x^0
    \end{cases}
\end{equation*}

为了计算传播子，我们将场算符的表达式代入。利用产生湮灭算符的性质 $\hat{a}_{\vec{p}}\ket{0} = 0$ 和 $\bra{0}\hat{a}_{\vec{p}}^\dagger = 0$，我们可以得到
\begin{align*}
    D_F(x-y) &= \int \frac{d^3p}{(2\pi)^3} \frac{1}{2E_{\vec{p}}} e^{-ip\cdot(x-y)} \theta(x^0-y^0) \\
    &\quad + \int \frac{d^3p}{(2\pi)^3} \frac{1}{2E_{\vec{p}}} e^{ip\cdot(x-y)} \theta(y^0-x^0)
\end{align*}

其中 $\theta$ 是阶跃函数 (step function)。为了将其写成相对论协变的形式，我们利用围道积分 (contour integral) 的技巧，最终得到
\begin{equation*}
...
传播子的物理意义是：它给出了一个虚粒子 (virtual particle) 从时空点 $y$ 传播到时空点 $x$ 的概率振幅。在动量空间中，传播子 $\tilde{D}_F(p) = \frac{i}{p^2 - m^2 + i\epsilon}$ 是Klein-Gordon算符 $(\partial^2 + m^2)$ 的格林函数 (Green's function)，满足
\begin{equation*}
    (\partial^2 + m^2) D_F(x-y) = -i\delta^{(4)}(x-y)
\end{equation*}

这个方程表明传播子是非齐次Klein-Gordon方程的解，其中 $\delta^{(4)}(x-y)$ 是四维狄拉克δ函数。

\subsection{\protect\hyperlink{:}{传播子的另一种表示}}
\addtocontents{toc}{\protect\hypertarget{:}{}}

传播子还可以用另一种形式表示，即通过留数定理 (residue theorem) 计算动量积分中的 $p^0$ 部分，得到
\begin{equation*}
    D_F(x-y) = \int \frac{d^3p}{(2\pi)^3} \frac{1}{2E_{\vec{p}}} e^{-iE_{\vec{p}}(x^0-y^0)} e^{i\vec{p}\cdot(\vec{x}-\vec{y})}
\end{equation*}

这种形式更清楚地显示了传播子与能量动量色散关系 $E_{\vec{p}} = \sqrt{\vec{p}^2 + m^2}$ 的联系。

\subsection{\protect\hyperlink{:}{因果性与传播子}}
\addtocontents{toc}{\protect\hypertarget{:}{}}

在相对论性量子场论中，因果性 (causality) 是一个基本要求。对于类空间隔 (spacelike separation) 的两个点，即 $(x-y)^2 < 0$，可观测量的对易关系必须为零，以保证信息的传播不能超光速。

对于Klein-Gordon场，我们可以证明当 $(x-y)^2 < 0$ 时，传播子 $D_F(x-y)$ 并不严格为零，但是对易子 $[\hat{\phi}(x), \hat{\phi}(y)]$ 在类空间隔处为零。这表明虽然传播子在类空间隔处不为零，但是可观测量的对易关系仍然满足因果性要求。

具体来说，对易子定义为
\begin{equation*}
    [\hat{\phi}(x), \hat{\phi}(y)] = \int \frac{d^3p}{(2\pi)^3} \frac{1}{2E_{\vec{p}}} \left(e^{-ip\cdot(x-y)} - e^{ip\cdot(x-y)}\right)
\end{equation*}
